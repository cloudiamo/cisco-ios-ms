...
COPY --chown=1000:1000 . /opt/fmc_repository/cisco-ios-ms
RUN install_repo_deps.sh /opt/fmc_repository/cisco-ios-ms/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/cisco-ios-ms/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
